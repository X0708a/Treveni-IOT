...
const appData = {
  farmer: {
    name: "ರಾಜೇಶ್ ಕುಮಾರ್ ಗೌಡ",
    name_english: "Rajesh Kumar Gowda",
    name_hindi: "राजेश कुमार गौड़ा",
    farm_name: "ಶ್ರೀ ಲಕ್ಷ್ಮೀ ಪೋಲ್ಟ್ರಿ ಫಾರ್ಮ್",
    farm_name_english: "Sri Lakshmi Poultry Farm",
    farm_name_hindi: "श्री लक्ष्मी पोल्ट्री फार्म",
    farm_id: "KA-BLR-002",
    phone: "+91 9845123456",
...
function initializeApp() {
  ...
  setTimeout(() => {
    showToast('🏠 ಕಿಸಾನ್ ಸಹಾಯಕ್‌ಗೆ ಸುಸ್ವಾಗತ - Welcome to Kisan Sahayak', 'success');
  }, 1000);
...
function getTabDisplayName(tabName) {
  const names = {
    ...
    scan: 'ಸ್ಕ್ಯಾನ್ - Scan',
    alerts: 'ಎಚ್ಚರಿಕೆಗಳು - Alerts',
    schedule: 'ವೇಳಾಪಟ್ಟಿ - Schedule',
    more: 'ಇನ್ನಷ್ಟು - More'
  };
...
function getScanTypeText(type) {
  const types = {
    visitor: 'ಭೇಟಿ ನೀಡುವವರ ಬ್ಯಾಡ್ಜ್ - Visitor Badge',
    equipment: 'ಉಪಕರಣ - Equipment',
    feed: 'ಮೇವು - Feed'
  };
  return types[type] || 'ಅಪರಿಚಿತ - Unknown';
}
...
function showScanResult(success) {
  ...
  if (success) {
    ...
    resultTitle.innerHTML = 'ಸ್ಕ್ಯಾನ್ ಯಶಸ್ವಿಯಾಗಿದೆ - Scan Successful';
    resultMessage.innerHTML = `${typeText} ಯಶಸ್ವಿಯಾಗಿ ಸ್ಕ್ಯಾನ್ ಮಾಡಲಾಗಿದೆ. ಎಲ್ಲಾ ನಿಯಮಗಳನ್ನು ಅನುಸರಿಸಲಾಗಿದೆ.`;
    showToast('✅ ಸ್ಕ್ಯಾನ್ ಯಶಸ್ವಿಯಾಗಿದೆ - Scan successful', 'success');
  } else {
    resultIcon.innerHTML = '❌';
    resultTitle.innerHTML = 'ಸ್ಕ್ಯಾನ್ ವಿಫಲವಾಗಿದೆ - Scan Failed';
    resultMessage.innerHTML = `${typeText} ಸ್ಕ್ಯಾನ್‌ನಲ್ಲಿ ಸಮಸ್ಯೆ. ದಯವಿಟ್ಟು ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.`;
    showToast('❌ ಸ್ಕ್ಯಾನ್ ವಿಫಲವಾಗಿದೆ - Scan failed', 'error');
  }
...
function callEmergency(type) {
  ...
  let message = `📞 ಪಶು ವೈದ್ಯರಿಗೆ ಕರೆ: ${contact.name} (${contact.phone})`;
  
  showToast(message, 'warning');
  simulateHapticFeedback('success');
  
  setTimeout(() => {
    showToast('📱 ಕರೆ ಸಂಪರ್ಕಗೊಂಡಿದೆ - Call connected', 'success');
  }, 2000);
}
...
function openHelp() {
  showToast('❓ ಸಹಾಯ ಕೇಂದ್ರ ತೆರೆಯಲಾಗುತ್ತಿದೆ - Opening help center', 'info');
  simulateHapticFeedback();
}

function openSettings() {
  showToast('🌐 ಭಾಷಾ ಸೆಟ್ಟಿಂಗ್‌ಗಳು ತೆರೆಯಲಾಗುತ್ತಿದೆ - Opening language settings', 'info');
  simulateHapticFeedback();
}
...
window.addEventListener('error', function(e) {
  ...
  showToast('⚠️ ಏನೋ ತಪ್ಪಾಗಿದೆ - Something went wrong. Please try again.', 'error');
});

window.addEventListener('unhandledrejection', function(e) {
  console.error('❌ Unhandled Promise:', e.reason);
  showToast('⚠️ ಸಂಪರ್ಕ ಸಮಸ್ಯೆ - Connection issue. Please check network.', 'warning');
});

// Final initialization log
console.log('🌾 Kisan Sahayak - Smallholder Farmer App Initialized Successfully');
console.log('📱 Frame Size: 414x896px | Language: Kannada + English | Features: All Active');